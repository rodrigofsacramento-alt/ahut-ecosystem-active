...
const INITIAL_TICKETS: TechTicket[] = [
  ...
  },
  {
    ...
    description: 'Implementar tela de gestão de chamados com esteira kanban de 4 estágios e agente conversacional empático para atendimento de colaboradores leigos.',
    module: 'Frontend & UI',
    ...
    acceptance_criteria: [
      ...
      'Assistente de Triagem com upload de imagem e gravação de áudio',
    ...
    timeline: [
      { status: 'Triado por IA', date: '19/08 09:20', user: 'Agente Triagem IA' },
  ...
  },
  {
    ...
    description: 'Planejar nova rotina de conciliação de mensagens com fila assíncrona para suportar picos de 500 mensagens/min sem delay no chat.',
    module: 'Backend & WhatsApp',
    requesterName: 'Juliana Costa',
    requesterRole: 'Supervisora de Atendimento',
    requesterDepartment: 'Recepção / SAC',
    priority: 'alta',
    main_status: 'executando',
    ...
    impact_level: 'Alto',
    is_ai_triaged: true,
    business_impact: 'Evita perda de mensagens de pacientes em horários de pico comercial.',
    acceptance_criteria: [
    ...
    timeline: [
      { status: 'Triado por IA', date: '19/08 08:05', user: 'Agente Triagem IA' },
  ...
  },
  {
    id: 'ticket-105',
    code: 'TCK-2026-085',
    title: 'Botão de Reagendamento Direto pelo Chat com Notificação SMS',
    description: 'As secretárias precisam de um atalho rápido na janela de conversa para remarcar consulta sem precisar trocar de aba para a Agenda.',
    module: 'Central de Chat & Agenda',
    requesterName: 'Ana Beatriz',
    requesterRole: 'Atendente Sênior',
    requesterDepartment: 'Recepção',
    priority: 'media',
    ...
    impact_level: 'Médio',
    is_ai_triaged: true,
    business_impact: 'Economiza cerca de 1 a 2 minutos por atendimento de reagendamento na recepção.',
    acceptance_criteria: [
      'Mini-modal de reagendamento acionável direto na conversa do paciente',
      'Confirmação automática enviada por WhatsApp ao paciente'
    ],
    attachments: [
      { id: 'att-5', name: 'audio_explicacao_anabeatriz.mp3', type: 'audio', url: '#', duration: '1:15' }
    ],
    timeline: [
      { status: 'Chamado Aberto com Agente IA', date: '19/08 11:20', user: 'Agente Triagem IA' },
  ...
  },
  {
    id: 'ticket-106',
    code: 'TCK-2026-086',
    title: 'Filtro por Faixa Etária e Histórico Clínico no Módulo Pacientes',
    description: 'Permitir filtragem rápida de pacientes pediátricos vs. geriátricos na listagem para campanhas de prevenção.',
    module: 'Pacientes',
    requesterName: 'Dra. Camila Alves',
    requesterRole: 'Médica Colaboradora',
    requesterDepartment: 'Corpo Clínico',
    priority: 'baixa',
    ...
    impact_level: 'Baixo',
    is_ai_triaged: true,
    business_impact: 'Facilita a extração de dados para pesquisas e ações preventivas.',
    acceptance_criteria: [
      'Dropdown com seletores de idade pré-configurados (0-12, 13-59, 60+)',
      'Exportação da lista filtrada em Excel/CSV'
    ],
    timeline: [
      { status: 'Triado por IA', date: '19/08 10:05', user: 'Agente Triagem IA' },
      { status: 'Aprovado e movido para A Executar', date: '19/08 10:45', user: 'Squad Ahut Tech' }
    ]
  }
];

const LOCAL_STORAGE_KEY = 'ahut_crm_tech_tickets_v2';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-between gap-4 bg-[#0d1321]/70 p-4 rounded-xl border border-gray-800/80 backdrop-blur-md">
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            placeholder="Buscar por código (TCK-...), título, solicitante, módulo ou impacto..."
    ...
    <div className="group relative bg-[#111827]/90 hover:bg-[#151e33] border border-gray-800 hover:border-sky-500/40 rounded-xl p-3.5 transition-all duration-200 shadow-md hover:shadow-sky-500/5 flex flex-col justify-between gap-3">
      ...
      {hasAttachments && (
        <div className="flex items-center gap-2 pt-1">
          {ticket.attachments?.filter(a => a.type !== 'audio').length ? (
            <span className="text-[9px] text-gray-400 flex items-center gap-1 bg-gray-900/90 px-2 py-0.5 rounded border border-gray-800">
              ...
              {ticket.attachments?.filter(a => a.type !== 'audio').length} anexo(s)
            </span>
          ) : null}
...
function AiTicketIntakeModal({
  ...
  const handleSendUserMessage = (textToSend?: string) => {
    ...
    setTimeout(() => {
      ...
      let aiReply = '';
      let newScore = understandingScore;
      ...
      if (turnCount === 1) {
        newScore = isLeadDist ? 70 : 55;
        aiReply = isLeadDist 
          ? 'Excelente ideia! A distribuição automática de leads por score/desempenho após 15 min evita ociosidade. Me conta: caso o corretor líder esteja offline ou com 5 atendimentos em aberto, o sistema deve passar a vez para o próximo da fila?'
          : 'Entendi perfeitamente o cenário! Me explica como isso afeta a rotina da equipe: quando você tenta fazer isso hoje, o que acontece de errado ou o que falta para ficar ideal?';
        suggestions = isLeadDist 
          ? ['Sim, passar para o próximo se estiver ocupado', 'Apenas notificar no WhatsApp', 'Manter a proporção 70/30']
          : ['Demora muito tempo manualmente', 'Gera confusão nos atendimentos', 'Falta um botão direto de ação', 'Gostaria de aviso sonoro'];
      } else if (turnCount === 2) {
        newScore = 85;
        aiReply = 'Ótimo detalhe! Ficou muito claro. Se você tiver algum print de exemplo ou quiser gravar um áudio de voz rápido explicando a regra, pode anexar agora. Além disso: quem são as pessoas da equipe que mais vão acompanhar essa métrica?';
        suggestions = ['Diretoria e Gestores de Vendas', 'Toda a equipe de Corretores', 'Administradores do CRM'];
      } else {
        newScore = 100;
        aiReply = 'Perfeito! Consegui compreender tudo com total clareza. Já estruturei o diagnóstico completo do chamado com os critérios de aceite e o impacto no sistema. Confira o resumo ao lado e confirme para subirmos o ticket para a equipe de tecnologia!';
        setIsReadyToConfirm(true);

        const summaryTitle = isLeadDist
          ? 'Distribuição Automática de Leads por Performance (15min timeout)'
          : (text.length > 40 ? text.slice(0, 45) + '...' : `Melhoria solicitada: ${text}`);

        const detectedModule = isLeadDist 
          ? 'Leads & Distribuição Automática' 
          : (lowerText.includes('chat') || lowerText.includes('whatsapp') ? 'Central de Atendimento / WhatsApp' : lowerText.includes('lead') ? 'Leads & CRM' : lowerText.includes('imóve') || lowerText.includes('imove') ? 'Imóveis & Catálogo' : lowerText.includes('agenda') ? 'Agenda & Visitas' : 'Frontend & Operações');

        setExtractedTicket({
          title: summaryTitle,
          description: isLeadDist 
            ? `Implementar regra de timeout de 15 minutos para distribuição automática de leads novos quando não distribuídos manualmente pelo admin. O algoritmo deve calcular o score de conversão/desempenho dos corretores ativos e distribuir proporcionalmente (ex: 70% líder, 30% vice-líder).`
            : `Solicitação detalhada de atualização para otimização da rotina: ${text}. Validado pelo colaborador com foco em agilidade e redução de atritos operacionais.`,
          module: detectedModule,
          business_impact: isLeadDist
            ? 'Elimina tempo de espera do lead sem atendimento inicial, premia corretores de alta performance e aumenta a taxa de conversão em vendas.'
            : 'Otimiza o fluxo de trabalho da equipe, reduz cliques repetitivos e previne erros operacionais.',
          priority: isLeadDist || lowerText.includes('urgente') || lowerText.includes('travou') ? 'alta' : 'media',
          acceptance_criteria: isLeadDist ? [
            'Cron job/worker checando leads sem responsável há mais de 15 min',
            'Cálculo dinâmico de taxa de conversão dos últimos 30 dias',
            'Distribuição ponderada na proporção de desempenho dos corretores online',
            'Notificação push e WhatsApp alertando o corretor contemplado'
          ] : [
            'Interface intuitiva sem necessidade de múltiplos passos',
            'Confirmação visual da ação realizada',
            'Compatibilidade mantida com os demais módulos'
          ]
        });
      }
  ...
  const handleFinalSubmit = () => {
    ...
    const newTicket: TechTicket = {
      ...
      requesterName: 'Rodrigo Sacramento',
      requesterRole: 'Colaborador',
      requesterDepartment: 'Recepção / Operações',
              ...
              {(!ticket.attachments || ticket.attachments.length === 0) ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {ticket.attachments.map(att => (
                    <div key={att.id} className="p-3.5 bg-gray-900 border border-gray-800 rounded-xl flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-sky-500/10 rounded-lg text-sky-400">
                        ...
                        <div>
                          <h5 className="font-semibold text-gray-200 truncate max-w-[200px]">{att.name}</h5>
