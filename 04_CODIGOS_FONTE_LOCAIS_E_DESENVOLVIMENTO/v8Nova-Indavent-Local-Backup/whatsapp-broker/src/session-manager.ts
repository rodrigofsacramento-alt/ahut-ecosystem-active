...
async function ensureWhatsappLead(params: {
  ...
  const phoneForSearch = phone.length > 11 && phone.startsWith('55') ? phone.slice(2) : phone;

  const { data: existingLead, error: lookupError } = await supabase
    .from('leads')
    .select('id, name, phone, stage, source_details')
    .ilike('phone', `%${phoneForSearch}%`)
    .maybeSingle();
  ...
  if (existingLead?.id) {
    ...
    const updatePayload: Record<string, any> = {
      phone,
      source_details: existingLead.source_details || 'WhatsApp',
      updated_at: now,
    };
...
async function shouldRunSafeProfilePicJob(job: SafeProfilePicJob) {
  const { data, error } = await supabase
    .from('whatsapp_contacts')
    .select('profile_pic_url, profile_pic_status, profile_pic_last_attempt_at, profile_pic_attempts')
    .eq('tenant_id', job.session.tenant_id)
...
async function findOrCreateParticipantProfile(
  ...
  const promise = (async () => {
    ...
    const { data: profileByEmail, error: emailLookupError } = { data: null, error: null } as any;
...
const pendingConversations = new Map<string, Promise<{ profileId: string; conversationId: string | null | undefined; phone: string; agentId: string | null }>>();
...
async function findOrCreateWhatsappConversation(
  ...
  isFromMe?: boolean,
  sock?: WASocket
): Promise<{ profileId: string; conversationId: string | null | undefined; phone: string; agentId: string | null }> {
  ...
  const promise = (async () => {
    ...
    // Auto-Validação e Sanitização de LID para Mensagem de cada Lead
    if (!remoteJid.endsWith('@g.us') && sock) {
      const isLidPhone = phoneToStore.length >= 14 && (phoneToStore.startsWith('1') || phoneToStore.startsWith('2') || phoneToStore.startsWith('8'));
      const isLidJid = remoteJid.endsWith('@lid');
      
      if (isLidPhone || isLidJid) {
        try {
          const queryPhone = existingContact?.phone_number && !existingContact.phone_number.startsWith('1') ? existingContact.phone_number : phoneToStore;
          const [onWa] = await sock.onWhatsApp(queryPhone);
          if (onWa && onWa.exists && onWa.jid) {
            const cleanResolved = normalizePhone(onWa.jid);
            if (cleanResolved.length >= 10 && cleanResolved.length <= 13) {
              logger.info({ oldPhone: phoneToStore, resolvedPhone: cleanResolved }, '⚡ Auto-Healing: Telefone real E.164 verificado e resolvido no WhatsApp!');
              phoneToStore = cleanResolved;
            }
          }
        } catch (err: any) {
          logger.warn({ err: err?.message, phoneToStore }, 'Falha na auto-validação de telefone real no WhatsApp');
        }
      }
    }
    ...
    if (!profileId) {
      // Skipped email lookup since profiles does not have an email column
      profileId = null;
    }
    ...
    if (!conversationId) {
      ...
      agentId = existingConversation?.agent_id || null;
    }

    if (!conversationId) {
      // Nova conversa entra como 'open' para o frontend CRM (whatsapp_contacts) poder visualizar
      ...
      // ATUALIZAÇÃO CRUCIAL: Garantir que a tabela 'whatsapp_contacts' saiba do ID da nova conversa!
      await supabase
        .from('whatsapp_contacts')
    ...
    return { profileId, conversationId, phone: phoneToStore, agentId };
  })();
...
export async function startSession(session: SessionRecord) {
  ...
  sock.ev.on('messages.upsert', async (upsert: any) => {
    ...
    for (const msg of upsert.messages) {
      ...
      // Reduzimos de 24 horas para 1 hora para evitar overload na sincronizacao inicial
      // e impedir que o broker caia tentando processar milhares de mensagens antigas.
      const oneHourAgo = Date.now() - (60 * 60 * 1000);
      if (msgTimestampMs < oneHourAgo) {
        logger.info({ jid: msg.key.remoteJid, msgId: msg.key.id, msgTime: msgTimestampMs }, 'Mensagem de histórico (> 1h) ignorada para manter estabilidade');
        continue;
      }
      ...
      const { profileId, conversationId, agentId } = await findOrCreateWhatsappConversation(session, remoteJid, msg.pushName, realPhone, isFromMe, sock);
      ...
      if (isGroup && !isFromMe) {
        ...
        if (participantJid) {
          try {
            ...
            await ensureGroupParticipant(session.tenant_id, remoteJid, participantProfileId);
      ...
      if (!isFromMe) {
        ...
        // marcamos o status como 'open' (Aberto) para que a view whatsapp_contacts consiga puxar o conversation_id.
...
function generateWaveform(buffer: Buffer): Uint8Array {
  const samples = 64;
  ...
  for (let i = 0; i < samples; i++) {
    ...
    const val = Math.min(100, Math.max(10, Math.floor((avg / 255) * 100)));
    waveform[i] = val;
  }
  return waveform;
}
    ...
    if (content.startsWith('[Audio]')) {
      logger.info({ jid, url: urlLine }, 'Baixando e convertendo áudio para OGG Opus nativo do WhatsApp...');
      try {
        ...
        logger.info({ jid, originalSize: rawBuffer.length, convertedSize: oggBuffer.length, waveformLength: waveform.length }, 'Áudio convertido com sucesso para OGG Opus com Waveform! Enviando...');
        ...
        logger.error({ jid, url: urlLine, err: err?.message || err }, 'Falha na conversão de áudio, enviando via URL fallback');
...
/**
 * Converte um buffer de áudio (qualquer formato suportado pelo ffmpeg, como MP3, M4A, WEBM, OGG)
 * para o formato exigido pelo WhatsApp (OGG com codec Opus, mono 1 canal, 48kHz, VoIP profile).
 */
