...
function AiTicketIntakeModal({
  ...
  const handleSendUserMessage = (textToSend?: string) => {
    ...
    setTimeout(() => {
      ...
      if (turnCount === 1) {
        newScore = 55;
        aiReply = 'Entendi o cenário! Me explica como isso afeta o seu dia a dia ou o atendimento: quando você tenta fazer isso hoje, o que acontece de errado ou o que falta para ficar perfeito?';
        suggestions = ['Demora muito tempo para fazer manualmente', 'Gera confusão na recepção', 'Falta um botão direto de ação', 'Gostaria de aviso sonoro'];
      } else if (turnCount === 2) {
        newScore = 85;
        aiReply = 'Excelente detalhe! Ficou muito claro. Se você tiver algum print da tela ou quiser gravar um áudio rápido explicando, pode anexar agora. Além disso: quem são as pessoas da equipe que mais vão usar essa melhoria?';
        suggestions = ['Toda a equipe de recepção', 'Apenas os médicos', 'A diretoria e gerência', 'Todos os usuários'];
      } else {
        newScore = 100;
        ...
        const summaryTitle = text.length > 40 ? text.slice(0, 45) + '...' : `Melhoria solicitada: ${text}`;
        setExtractedTicket({
          title: summaryTitle,
          description: `Solicitação detalhada de atualização para otimização da rotina: ${text}. Validado pelo colaborador com foco em agilidade e redução de atritos operacionais.`,
          module: text.toLowerCase().includes('chat') || text.toLowerCase().includes('whatsapp') ? 'Central de Chat & WhatsApp' : text.toLowerCase().includes('paciente') ? 'Pacientes' : text.toLowerCase().includes('agenda') ? 'Agenda & Consultas' : 'Frontend & Operações',
          business_impact: 'Otimiza o fluxo de trabalho da equipe, reduz cliques repetitivos e previne erros operacionais.',
          priority: text.toLowerCase().includes('urgente') || text.toLowerCase().includes('travou') ? 'alta' : 'media',
          acceptance_criteria: [
