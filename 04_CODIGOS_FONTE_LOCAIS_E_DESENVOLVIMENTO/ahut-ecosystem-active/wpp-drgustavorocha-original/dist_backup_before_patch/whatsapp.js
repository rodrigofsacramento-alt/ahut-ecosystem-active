...
export async function startWhatsAppSession(tenantId, sessionName) {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        for (const msg of m.messages) {
            if (!msg.message || msg.key.fromMe)
                continue; // Ignorar mensagens enviadas por mim mesmo no app original
            const remoteJid = msg.key.remoteJid;
            if (!remoteJid || remoteJid.includes('@g.us'))
                continue; // Ignorar grupos por enquanto
...
export async function sendOutboundMessage(tenantId, phone, content) {
