...
class AiReportService:
    ...
    def _build_prompts(avaliado: dict, scores: dict) -> dict:
        ...
        mbti = scores.get("mbti", {})
        big_five = scores.get("big_five", {})
        ancoras = scores.get("ancoras", {})
        opq = scores.get("opq_scores", {})
        valores = scores.get("valores", {})
        confidence = scores.get("confidence_score", 0)

        # Build base context string
        mbti_type = mbti.get("type", "ENTJ")
        disc_primary = disc.get("primary", "D")
        disc_secondary = disc.get("secondary", "I")
...
RESULTADOS GERAIS:
DISC: D={disc.get('d',0)}%, I={disc.get('i',0)}%, S={disc.get('s',0)}%, C={disc.get('c',0)}% (Tipo: {disc_primary}/{disc_secondary})
MBTI: {mbti_type}
"""
...
BIG FIVE SCORES:
Abertura: {big_five.get('openness', 0)}%
Conscienciosidade: {big_five.get('conscientiousness', 0)}%
Extroversão: {big_five.get('extraversion', 0)}%
Amabilidade: {big_five.get('agreeableness', 0)}%
Neuroticismo: {big_five.get('neuroticism', 0)}%
...
"""
        ...
        prompts["relatorio_6_valores"] = base_context + """
