...
async def get_all_users():
    ...
    users_res = supabase.table("users").select("id, name, full_name, email, role, company, department, created_at").neq("role", "admin").execute()
    users = users_res.data if users_res.data else []
    
    # Busca todos os assessments para saber o status
    assessments_res = supabase.table("assessments").select("id, user_id, tool_name").execute()
    ...
    if assessments_res.data:
        for a in assessments_res.data:
            ...
            if uid not in assessments_map:
                assessments_map[uid] = []
            assessments_map[uid].append(a["tool_name"])
            last_assessment_map[uid] = a.get("id") # Em real db deveria agrupar por assessment_id e tool_name, mas simplificamos
