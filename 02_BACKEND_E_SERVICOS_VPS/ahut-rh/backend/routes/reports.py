...
async def generate_final_pdf(job_id: str, payload: Optional[dict] = None):
    ...
    try:
        ...
        # Validar que o assessment_id é um UUID válido antes de tentar salvar
        if not assessment_id or not UUID_RE.match(str(assessment_id)):
            print(f"assessment_id inválido ({assessment_id}), pulando salvamento no banco.")
        else:
            user_id = scores.get("user_id") if scores else None
            if not user_id or not UUID_RE.match(str(user_id)):
                # Tenta buscar pelo assessment usando .limit(1) para evitar 406
                assessment = supabase.table("assessments").select("user_id").eq("id", assessment_id).limit(1).execute()
                if assessment.data and len(assessment.data) > 0:
                    user_id = assessment.data[0].get("user_id")
            
            if user_id and UUID_RE.match(str(user_id)):
                import json
                data = {
                    "assessment_id": assessment_id,
                    "user_id": user_id,
                    ...
                    "pdf_url": pdf_url,
                    "content": results # Salva o JSON completo das 60 páginas para o HTML ler
                }
                # Salva na tabela oficial nova
                supabase.table("rh_behavioral_reports").upsert(data).execute()
                print(f"Relatório salvo com sucesso para assessment_id={assessment_id}")
            else:
                print(f"user_id inválido ou não encontrado, pulando salvamento no banco.")
    except Exception as e:
        print(f"Erro ao salvar PDF e JSON no banco: {e}")
